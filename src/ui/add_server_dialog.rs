...
pub fn show_server_dialog<F>(
    ...
    F: Fn(SshHost, String) + 'static,
{
    let dialog = gtk4::Dialog::builder()
        .transient_for(parent)
        .modal(true)
        .title(if initial_host.is_some() {
            "Edit Server"
        } else {
            "Add New Server"
        })
    ...
    let alias_entry = gtk4::Entry::builder()
        .placeholder_text("Alias (e.g. My VPS)")
        .build();
    let host_entry = gtk4::Entry::builder()
        .placeholder_text("Hostname or IP")
        .build();
    let port_entry = gtk4::Entry::builder()
        .placeholder_text("Port (default: 22)")
        .build();
    let user_entry = gtk4::Entry::builder()
        .placeholder_text("User (default: root)")
        .build();
    ...
    let ok_button = dialog.add_button(
        if initial_host.is_some() {
            "Save"
        } else {
            "Add"
        },
    ...
    alias_entry.connect_changed(move |e| {
        ...
        let is_duplicate =
            existing_aliases_clone.contains(&text) && Some(text.clone()) != initial_alias;
    ...
    dialog.connect_response(move |d, res| {
        if res == gtk4::ResponseType::Ok {
            ...
            let host = SshHost {
                ...
                user: Some(user_entry.text().to_string().trim().to_string())
                    .filter(|s| !s.is_empty()),
