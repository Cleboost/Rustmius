...
pub fn load_ssh_keys() -> Result<Vec<SshKey>, Box<dyn std::error::Error>> {
    ...
    for entry in entries {
        ...
        let entry = key_pairs.entry(base_name.to_string()).or_insert((None, None));
        if is_public {
...
pub fn regenerate_public_key(private_key_path: &str) -> Result<String, Box<dyn std::error::Error>> {
    ...
    if !output.status.success() {
        ...
        return Err(format!("Erreur lors de la génération de la clé publique: {}", error_msg).into());
    }
...
pub fn read_key_content(key_name: &str) -> Result<(Option<String>, Option<String>), Box<dyn std::error::Error>> {
    let ssh_dir = dirs::home_dir()
        .ok_or("Impossible de trouver le répertoire home")?
        .join(".ssh");
...
fn get_key_info(key_path: &Path) -> Result<KeyInfo, Box<dyn std::error::Error>> {
    ...
    let key_type = if content.contains("-----BEGIN RSA PRIVATE KEY-----") {
        ...
        for line in &lines {
            ...
            if !trimmed.is_empty() && !trimmed.starts_with('#') && !trimmed.starts_with("-----BEGIN") && !trimmed.starts_with("-----END") {
    ...
    let fingerprint = format!("SHA256:{}...", &key_path.file_name().unwrap_or_default().to_string_lossy()[..std::cmp::min(20, key_path.file_name().unwrap_or_default().to_string_lossy().len())]);
